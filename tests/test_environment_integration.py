...
def signal_description():
    return SignalDescription(
        t_start=0.0, t_stop=0.001, f_low=-5e3, f_high=5e3, label="QPSK", snr=20.0,
    )


class TestLinkParamsToImpairments:
    def test_returns_list_of_transforms(self):
        lp = LinkParams(emitter_index=0, snr_db=15.0, path_loss_db=100.0, received_power_dbm=-70.0, delay_s=3.3e-6, doppler_hz=0.0, distance_m=1000.0, fading_suggestion=None)
    ...
    def test_doppler_included_when_nonzero(self):
        lp = LinkParams(emitter_index=0, snr_db=15.0, path_loss_db=100.0, received_power_dbm=-70.0, delay_s=3.3e-6, doppler_hz=240.0, distance_m=1000.0, fading_suggestion=None)
        result = link_params_to_impairments(lp)
        doppler_list = [t for t in result if isinstance(t, DopplerShift)]
        assert len(doppler_list) == 1

    def test_rician_fading_from_suggestion(self):
        lp = LinkParams(emitter_index=0, snr_db=15.0, path_loss_db=100.0, received_power_dbm=-70.0, delay_s=3.3e-6, doppler_hz=0.0, distance_m=1000.0, fading_suggestion="rician_k6")
        result = link_params_to_impairments(lp)
    ...
    def test_rayleigh_fading_from_suggestion(self):
        lp = LinkParams(emitter_index=0, snr_db=15.0, path_loss_db=100.0, received_power_dbm=-70.0, delay_s=3.3e-6, doppler_hz=0.0, distance_m=1000.0, fading_suggestion="rayleigh")
        result = link_params_to_impairments(lp)
    ...
    def test_impairment_order_doppler_fading_awgn(self):
        lp = LinkParams(emitter_index=0, snr_db=15.0, path_loss_db=100.0, received_power_dbm=-70.0, delay_s=3.3e-6, doppler_hz=240.0, distance_m=1000.0, fading_suggestion="rayleigh")
        result = link_params_to_impairments(lp)
        assert len(result) == 3
...
class TestEndToEnd:
    def test_environment_to_impairments_apply(self, signal_description, assert_valid_iq):
        env = Environment(
            ...
            receiver=ReceiverConfig(position=Position(0.0, 0.0), noise_figure_db=6.0, bandwidth_hz=1e6),
        )
    ...
    def test_compose_wrapping(self, signal_description, assert_valid_iq):
        lp = LinkParams(emitter_index=0, snr_db=20.0, path_loss_db=80.0, received_power_dbm=-50.0, delay_s=1e-6, doppler_hz=0.0, distance_m=300.0, fading_suggestion=None)
