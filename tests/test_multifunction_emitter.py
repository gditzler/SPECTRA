...
def test_multifunction_search_track_radar_produces_two_modes(assert_valid_iq):
    ...
    # Search CPI = 32_768 samples, Track CPI = 4_096 samples, so we need at
    # least 36_864 samples for both modes to appear in a single pass.
...
def test_multifunction_search_track_radar_track_is_louder():
    ...
    sw = multifunction_search_track_radar()
    _, segs = sw.generate_with_segments(num_samples=60_000, sample_rate=20e6, seed=0)
