# S3CA Paper Reproduction Notebook Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create `examples/10_s3ca_paper_reproduction.ipynb` as a direct notebook translation of `examples/10_s3ca_paper_reproduction.py`.

**Architecture:** 1:1 cell mapping from the `.py` script, splitting at section comment boundaries into alternating markdown + code cells. Replace `matplotlib.use("Agg")` with `%matplotlib inline`. No new logic — only presentation changes.

**Tech Stack:** Jupyter notebook (`.ipynb` JSON format), matplotlib, numpy, spectra.

---

### Task 1: Create the notebook file

**Files:**
- Create: `examples/10_s3ca_paper_reproduction.ipynb`

**Step 1: Write the notebook**

Create `examples/10_s3ca_paper_reproduction.ipynb` with the following cells (use the existing `examples/09_s3ca_vs_ssca.ipynb` as the structural template for the JSON format):

**Cell 0 — Markdown (title/intro):**
```
# SPECTRA Example 10: S3CA Paper Reproduction (Li et al. IEEE SPL 2015)

Reproduces Figure 3 from **"S3CA: A Sparse Strip Spectral Correlation Analyzer"** by Carol Jingyi Li et al.
...
**Cell 9 — Code (full figure + kappa sweep):**
```python
...
ax1.set_title("(a) SSCA (Sx_SSCA)", fontsize=10)
...
In this notebook you reproduced Figure 3 from Li et al. (IEEE SPL 2015) using SPECTRA's S3CA implementation.

Key takeaways:
- **DSSS-BPSK** exhibits cyclostationary signatures at multiples of the data rate (≈ 0.00806 = chip_rate / processing_gain)
- **S3CA** with `kappa=80` recovers the dominant cycle frequencies while computing far fewer FFT rows than the full SSCA
- **Alpha profiles** reveal the comb-like cyclic structure characteristic of spread-spectrum signals
- **L1-norm vs κ** shows diminishing error as kappa grows — a small kappa already captures most of the signal energy
- The residual surface confirms that S3CA misses only low-energy, off-peak bins

Key parameters:
- `nfft` (Np): Channelizer band count — controls spectral frequency resolution
- `n_alpha`: Number of cyclic frequency bins — set to next power-of-2 ≥ n_frames
- `kappa`: Sparse samples per band — trades accuracy for speed
- `seed`: Ensures deterministic random row selection
```

**Step 2: Verify the notebook loads**

```bash
cd examples && jupyter nbconvert --to notebook --execute 10_s3ca_paper_reproduction.ipynb --output /tmp/test_out.ipynb 2>&1 | tail -5
```

Expected: exits 0, no tracebacks. (This step is optional/slow — skip if environment lacks jupyter.)

**Step 3: Commit**

```bash
git add examples/10_s3ca_paper_reproduction.ipynb docs/plans/2026-03-04-s3ca-paper-reproduction-notebook-design.md docs/plans/2026-03-04-s3ca-paper-reproduction-notebook.md
git commit -m "docs: add notebook for example 10 S3CA paper reproduction"
```
