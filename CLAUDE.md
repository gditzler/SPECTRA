...
- `environment/` — Geometry-driven link modeling. `Position` (3-D coordinates), `Emitter` (waveform + position + Tx params), `ReceiverConfig`, `Environment` (computes per-emitter `LinkParams`: SNR, path loss, delay, Doppler, RMS delay spread, K-factor). `link_params_to_impairments()` auto-wires AWGN + Doppler + Rayleigh/Rician + TDLChannel from a `LinkParams`. `propagation_presets` dict of common scenarios. Propagation models in `environment/propagation/`: `FreeSpacePathLoss`, `LogDistancePL`, `COST231HataPL`, `OkumuraHataPL`, `ITU_R_P525` (with optional P.676 gaseous absorption), `ITU_R_P1411` (short-range), and 3GPP 38.901 family (`GPP38901UMa`, `GPP38901UMi`, `GPP38901RMa`, `GPP38901InH`). All return `PathLossResult` with shadow fading, RMS delay spread, K-factor, and angular spread. YAML round-trip via a registry.
